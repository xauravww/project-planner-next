...
export default function NewProjectPage() {
    ...
    const append = async (message: { role: string; content: string }) => {
        ...
        try {
            // Include MCQ context in the request if available
            const answersArray = Object.entries(mcqAnswers)
                .filter(([_, selected]) => selected.length > 0)
                .map(([qId, selected]) => {
                    const question = DEFAULT_QUESTIONS.find(q => q.id === qId);
                    return `${question?.text}: ${selected.join(", ")}`;
                });
            ...
            const response = await fetch("/api/chat", {
                ...
                body: JSON.stringify({
                    messages: [...messages, { ...userMessage, content: userMessage.content + contextMessage }]
                }),
            });
