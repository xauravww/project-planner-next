        ...
        <div className="h-full overflow-y-auto pb-32 relative">
            ...
            <div className="container mx-auto px-4 pt-24">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
                    <div>
                        <h1 className="text-4xl md:text-5xl font-bold text-white mb-2 tracking-tight">
                            Welcome back, <span className="text-zinc-400">{session.user?.name?.split(" ")[0] || "Builder"}</span>
                        </h1>
                        <p className="text-lg text-muted-foreground">Here&apos;s what&apos;s happening with your projects today.</p>
                    </div>
                    <div className="flex items-center gap-4">
                        <Button variant="glass" className="h-12 px-6 hover:bg-white/10 text-zinc-300 hover:text-white border-white/10">
                            <Clock className="mr-2 h-4 w-4" />
                            Recent Activity
                        </Button>
                        <Link href="/dashboard/new">
                            <Button className="h-12 px-6 bg-white text-black hover:bg-zinc-200 border-0 shadow-lg shadow-white/5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
                    <GlassCard className="p-6 relative overflow-hidden group border-white/5 bg-white/[0.02]">
                    ...
                    {projects.length === 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            <Link href="/dashboard/new" className="group">
                                <GlassCard className="h-64 flex flex-col items-center justify-center text-center p-8 border-2 border-dashed border-white/10 hover:border-white/30 hover:bg-white/5 transition-all duration-300">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                            ...
                            <Link href="/dashboard/new" className="group">
                                <GlassCard className="h-full min-h-[280px] flex flex-col items-center justify-center text-center p-6 border-2 border-dashed border-white/10 hover:border-white/30 hover:bg-white/5 transition-all duration-300">
                            ...
                            {projects.map((project: any) => (
                                <Link key={project.id} href={`/projects/${project.id}`} className="group">
                                    <GlassCard className="h-full min-h-[280px] p-6 flex flex-col relative overflow-hidden hover:-translate-y-1 hover:shadow-2xl hover:shadow-white/5 transition-all duration-300 border-white/5 bg-white/[0.02]">
